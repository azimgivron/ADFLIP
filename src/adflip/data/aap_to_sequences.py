...
from adflip.data.all_atom_parse import index_to_token, load_structure_data, restype_3to1


def convert_aap_to_protein_sequences(
    structure_data_path: str, name: str = None
) -> List[Tuple[str, str]]:
    if name is None:
        name = os.path.basename(structure_data_path).split(".")[0]
    structure_data = load_structure_data(structure_data_path)
    protein_residues = structure_data.residue_token[
        structure_data.is_center * structure_data.is_protein
    ]
    chain_ids = structure_data.chain_id[
        structure_data.is_center * structure_data.is_protein
    ]
    extracted_sequences = []
    ...
    for chain_id in np.sort(np.unique(chain_ids)):
        ...
        extracted_sequence = "".join(
            [restype_3to1.get(index_to_token[x], "X") for x in chain_residues]
        )
