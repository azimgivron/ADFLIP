...
from PIPPack.data.protein import Protein, from_pdb_file
from PIPPack.model.loss import get_renamed_coords, masked_mean
...
def get_alt_CH(
    CH: torch.Tensor,
    S: torch.Tensor,
    pseudo_periodic: bool = False,
    return_periodic_chi: bool = False,
) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    # Determine which residues have a periodic chi angle.
    if pseudo_periodic:
        periodic_chi = torch.tensor(rc.chi_pseudo_pi_periodic, device=S.device)[
            S.long()
        ]
    else:
    ...
    alt_CH = CH.clone()
    alt_CH += torch.where(
        CH * periodic_chi > 0.0, -torch.pi * periodic_chi, torch.pi * periodic_chi
    )
...
def compute_chi_error(
    native_protein: Protein,
    decoy_protein: Protein,
    sc_b_factor_cutoff: float = 1000,
    compute_pseudo_ae: bool = False,
    ...
    rotamer_mask[torch.sum(chi_aatype_mask, -1) == 0] = 0.0
    ...
    angle_diff[angle_diff < -torch.pi] = (
        angle_diff[angle_diff < -torch.pi] + 2 * torch.pi
    )

    # Compute alternative angle difference (in radians), accounting for periodicity
    alt_angle_diff = native_CH - decoy_alt_CH
    alt_angle_diff[alt_angle_diff > torch.pi] = (
        alt_angle_diff[alt_angle_diff > torch.pi] - 2 * torch.pi
    )
    alt_angle_diff[alt_angle_diff < -torch.pi] = (
        alt_angle_diff[alt_angle_diff < -torch.pi] + 2 * torch.pi
    )
    ...
    if compute_pseudo_ae:
        ...
        decoy_CH = decoy_CH * (1.0 - alt_better) + decoy_alt_CH * alt_better
        ...
        pseudo_alt_angle_diff[pseudo_alt_angle_diff > torch.pi] = (
            pseudo_alt_angle_diff[pseudo_alt_angle_diff > torch.pi] - 2 * torch.pi
        )
        pseudo_alt_angle_diff[pseudo_alt_angle_diff < -torch.pi] = (
            pseudo_alt_angle_diff[pseudo_alt_angle_diff < -torch.pi] + 2 * torch.pi
        )
...
def compute_centrality(
    ...
    coords[~torch.isfinite(torch.sum(coords, dim=-1))] = protein_X[
        ..., rc.atom_order[backup_atom], :
    ][~torch.isfinite(torch.sum(coords, dim=-1))]
...
def find_unclosed_proline(
    protein, tolerance_factor=12, device=torch.device("cpu")
) -> torch.Tensor:
    ...
    pro_N = (
        pro_mask[..., None]
        * protein_X[..., rc.restype_name_to_atom14_names["PRO"].index("N"), :]
    )
    pro_CD = (
        pro_mask[..., None]
        * protein_X[..., rc.restype_name_to_atom14_names["PRO"].index("CD"), :]
    )
    pro_CD_N = torch.norm(pro_CD - pro_N, dim=-1)  # [..., L]

    # Find unclosed prolines based on tolerance factor
    dists = F.relu(
        pro_CD_N - (pro_CD_N_mean + tolerance_factor * pro_CD_N_std)
    )  # [..., L]
...
def compute_sc_rmsd(
    ...
    decoy_protein: Protein,
    sc_b_factor_cutoff: float = 1000,
    compute_pseudo_rmsd: bool = False,
    per_res: bool = True,
    device=torch.device("cpu"),
) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
    ...
    rmsd_renamed = torch.sqrt(
        masked_mean(atom_mask, torch.nan_to_num(renamed_atom_deviation), dim)
    )
    ...
    if compute_pseudo_rmsd:
        ...
        decoy_pseudo_renamed_X = get_renamed_coords(
            decoy_X, decoy_S, pseudo_renaming=True
        )
        pseudo_renamed_atom_deviation = torch.sum(
            torch.square(native_X - decoy_pseudo_renamed_X), dim=-1
        )
        rmsd_pseudo_renamed = torch.sqrt(
            masked_mean(atom_mask, torch.nan_to_num(pseudo_renamed_atom_deviation), dim)
        )
        rmsd_pseudo = torch.minimum(rmsd, rmsd_pseudo_renamed)
...
def compute_pairwise_vdw_table_and_masks(
    ...
    atomtype_radius = torch.tensor(
        rc.restype_atom_radius_atom14, dtype=torch.float32, device=sequence.device
    )[sequence]
    ...
    if len(sequence.shape) == 2:
        batch_index = torch.arange(
            sequence.shape[0], dtype=torch.long, device=sequence.device
        )
    ...
    residue_index = residue_index.repeat(
        *((1,) * len(residue_index.shape[:-1])), atom_mask.shape[-1]
    )
    ...
    vdw_mask = residue_index[:, None] < residue_index[None, :]
    ...
    sg_mask[
        sequence == rc.restype_order["C"],
        rc.restype_name_to_atom14_names["CYS"].index("SG"),
    ] = 1.0
    ...
    hbond_donors = torch.tensor(rc.restype_hbond_donors_atom14, device=sequence.device)[
        sequence
    ][atom_mask.bool()]
    hbond_acceptors = torch.tensor(
        rc.restype_hbond_acceptors_atom14, device=sequence.device
    )[sequence][atom_mask.bool()]
...
def compute_clashes(
    ...
    vdw_table, vdw_mask, hbond_mask = compute_pairwise_vdw_table_and_masks(
        sequence, atom_mask
    )

    # Get list of tolerance fractions
    tol_fracs = (
        global_tol_frac if isinstance(global_tol_frac, list) else [global_tol_frac]
    )
...
def assess_sidechains(
    ...
    decoy_pdb_path: str,
    sc_b_factor_cutoff: float = 1000,
    clash_tolerances: Sequence[float] = [0.8, 0.9, 1.0],
    hbond_allowance: float = 0.6,
    convert_mse: bool = False,
    device=torch.device("cpu"),
):
    # Load native protein
    ...
    chi_error = compute_chi_error(
        native_protein, decoy_protein, sc_b_factor_cutoff, device=device
    )
...
def summarize(stats, per_aatype: bool = False):
    ...
    total_stats = {
        "chi_error": {
            key: torch.cat(
                [stats[target]["chi_error"][key] for target in stats], dim=-2
            )
            for key in stats[list(stats.keys())[0]]["chi_error"]
        },
        "centrality": torch.cat(
            [stats[target]["centrality"] for target in stats], dim=-1
        ),
        "rmsd": {
        ...
        "clash_info": {
            tol: {
                "num_clashes": np.mean(
                    [
                        stats[target]["clash_info"][tol]["num_clashes"].cpu()
                        for target in stats
                    ]
                ).item(),
                "loss_avg": np.mean(
                    [
                        stats[target]["clash_info"][tol]["loss_avg"].cpu()
                        for target in stats
                    ]
                ).item(),
            }
        ...
        "unclosed_pro": np.sum(
            [stats[target]["unclosed_pro"].cpu().item() for target in stats]
        ),
    ...
    for aatype in aatypes:
        ...
        if aatype != "all":
            aatype_mask = (
                total_stats["seq"] == rc.restype_order[rc.restype_3to1[aatype]]
            )
        ...
        for centrality in ["all", "core", "surface"]:
            ...
            chi_ae_deg = total_stats["chi_error"]["chi_mae"].clone()[
                centrality_mask
            ] * (180.0 / torch.pi)
            chi_mae = masked_mean(
                chi_mask, chi_ae_deg, dim=list(range(chi_mask.dim() - 1))
            )

            # Rotamer Recovery
            has_rotamer = (
                total_stats["chi_error"]["rotamer_mask"]
                .clone()[centrality_mask]
                .squeeze(-1)
            )
            ...
            # RMSD
            mean_rmsd = masked_mean(
                total_stats["rmsd"]["sc_mask"].clone()[centrality_mask],
                total_stats["rmsd"]["sc_rmsd"].clone()[centrality_mask],
            )
            ...
            centrality_dict = {
                ...
                "num_chi": torch.sum(chi_mask, dim=list(range(chi_mask.dim() - 1)))
                .cpu()
                .numpy(),
            }
...
def main(
    ...
    decoy_dir: str,
    decoy_tag: str = "",
    out_filename: str = "packing_stats",
    sc_b_factor_cutoff: float = 1000,
    clash_tolerances: str = "0.8,0.9,1.0",
    hbond_allowance: float = 0.6,
    convert_mse: bool = False,
    per_aatype: bool = False,
    truncate: int = -1,
    verbose: bool = False,
):
    # If have gpu, use it
    ...
    for target in target_list:
        ...
        target_stats = assess_sidechains(
            native_pdb,
            decoy_pdb,
            sc_b_factor_cutoff,
            clash_tolerances,
