...
def main(args):
    ...
    from proteinmpnn.model_utils import (
        ProteinMPNN,
        featurize,
        get_std_opt,
        loss_nll,
    ...
    with ProcessPoolExecutor(max_workers=12) as executor:
        ...
        dataset_test = StructureDataset(
            pdb_dict_test, truncate=None, max_length=args.max_protein_length
        )
        ...
        for r in range(args.num_repeats):
            ...
            with torch.no_grad():
                ...
                test_acc = {"all": [0.0, 0], "core": [0.0, 0], "surface": [0.0, 0]}
                aatype_acc = {aatype: [0.0, 0] for aatype in aatype_accuracy}
                for _, batch in enumerate(loader_test):
                    ...
                    Cb = (
                        -0.58273431 * a + 0.56802827 * b - 0.54067466 * c + X[..., 1, :]
                    )
                    ...
                    test_acc["core"][0] += (
                        torch.sum(true_false * mask_for_loss * centrality_mask)
                        .cpu()
                        .data.numpy()
                    )
                    test_acc["core"][1] += (
                        torch.sum(mask_for_loss * centrality_mask).cpu().data.numpy()
                    )
                    ...
                    for aatype in aatype_acc:
                        ...
                        aatype_acc[aatype][0] += (
                            torch.sum(true_false * mask_for_loss * aatype_mask)
                            .cpu()
                            .data.numpy()
                        )
                        aatype_acc[aatype][1] += (
                            torch.sum(mask_for_loss * aatype_mask).cpu().data.numpy()
                        )
            ...
            for centrality in centrality_accuracy:
                centrality_accuracy[centrality].append(
                    test_acc[centrality][0] / test_acc[centrality][1]
                )

            for aatype in aatype_accuracy:
                aatype_accuracy[aatype].append(
                    aatype_acc[aatype][0] / aatype_acc[aatype][1]
                )
        ...
        for aatype in aatype_accuracy:
            print(
                f"\t{aatype}: {np.mean(aatype_accuracy[aatype]):.5f} +- {np.std(aatype_accuracy[aatype]):.5f}"
            )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--reload_data_every_n_repeats",
        type=int,
        default=1,
        help="how often to reload data",
    )
    argparser.add_argument(
    ...
    argparser.add_argument(
        "--n_points", type=int, default=8, help="number of points for IPMP"
    )
    argparser.add_argument(
