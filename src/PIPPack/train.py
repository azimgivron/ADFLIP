...
from PIPPack.model.loss import MetricLogger
from PIPPack.model.optim import get_std_opt
from PIPPack.utils.train_utils import determine_best_epoch_from_log, load_checkpoint

# Library code
from PIPPack.utils.utils import count_parameters, create_subdirs

logger = logging.getLogger(__name__)


def train_loop(
    exp_cfg, model, optimizer, train_dataloader, valid_dataloader, metric_logger, device
) -> None:
    ...
    for e in range(1, exp_cfg.epochs + 1):
        ...
        for batch in train_dataloader:
            ...
            loss = model.compute_loss(
                output,
                batch,
                use_sc_bf_mask=exp_cfg.use_b_factor_mask,
                _logger=metric_logger,
            )
...
def main(cfg: DictConfig) -> None:
    ...
    device = torch.device(
        "cuda:0"
        if (torch.cuda.is_available() and not cfg.experiment.force_cpu)
        else "cpu"
    )
    ...
    if "weights_path" in cfg.experiment:
        ...
        checkpoint = os.path.join(
            cfg.experiment.weights_path, f"{cfg.experiment.model_name}_ckpt.pt"
        )
    ...
    best_results = determine_best_epoch_from_log(
        log_file, [model.monitor_metric], highest=False, delimiter=","
    )
    best_ckpt = os.path.join(
        os.getcwd(), "checkpoints", f"epoch_{best_results['epoch']}.pt"
    )
    best_ckpt_copy = os.path.join(
        os.getcwd(), "results", f"best_ckpt_epoch_{best_results['epoch']}.pt"
    )
