...
def sample_epoch(
    ...
    chi_bin_one_hot = torch.nn.functional.one_hot(
        chi_bin, num_classes=model.n_chi_bins + 1
    )
    ...
    if chi_bin_offset is not None:
        ...
        bin_sample_update = (2 * torch.pi / model.n_chi_bins) * torch.rand(
...
@hydra.main(
    version_base=None, config_path="./config", config_name="inference_ensemble_sc"
)
def main(cfg: DictConfig) -> None:
    ...
    for model_name in cfg.inference.model_names:
        ...
        with open(
            os.path.join(cfg.inference.weights_path, f"{model_name}_config.pickle"),
            "rb",
        ) as f:
