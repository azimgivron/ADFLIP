...
rotamer_test = (
    "/nas/longleaf/home/nzrandol/kuhl_lab/MolProbity/molprobity/cmdline/rotamer-test"
)
...
def _eval_rotamers(pdb_file, out_file, lock):
    ...
    tmp_file = os.path.join(
        os.path.dirname(out_file), f"tmp_eval_rotamers_{os.getpid()}.txt"
    )
...
def main(args):
    ...
    with open(os.path.join(args.pdb_dir, "rotamer_evals.txt"), "a") as f:
        ...
        f.write(
            f"\tOutliers: {rotamer_evals['outliers']} ({100 * rotamer_evals['outliers'] / rotamer_evals['num_residues']:.3f}%)\n"
        )
        f.write(
            f"\tAllowed: {rotamer_evals['allowed']} ({100 * rotamer_evals['allowed'] / rotamer_evals['num_residues']:.3f}%)\n"
        )
        f.write(
            f"\tFavored: {rotamer_evals['favored']} ({100 * rotamer_evals['favored'] / rotamer_evals['num_residues']:.3f}%)\n"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pdb_dir", type=str, help="Directory containing pdb files to evaluate."
    )
