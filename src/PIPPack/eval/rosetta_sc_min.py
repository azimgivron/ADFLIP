...
def main(args):
    ...
    with Pool(args.n_thread) as p:
        p.map(
            partial(safe_run, fa_rep_weight=args.fa_rep, restrict_aa=args.restrict_aa),
            work_list,
        )
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Rosetta Side Chain Minimization",
    ...
    if args.out_dir is None:
        args.data_dir = (
            args.data_dir[:-1] if args.data_dir[-1] == os.path.sep else args.data_dir
        )
