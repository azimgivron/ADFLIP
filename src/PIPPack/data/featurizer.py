...
class Featurizer(object):
    ...
    def __call__(self, protein: FeatureDict) -> FeatureDict:
        ...
        bb_dihedrals, bb_dihedrals_mask = self.calc_bb_dihedrals(
            protein["atom_positions"]
        )
        bb_dihedrals_sincos = self._sincos_encoding(bb_dihedrals)
        sc_dihedrals, sc_dihedrals_mask = self.calc_sc_dihedrals(
            protein["atom_positions"], protein["aatype"]
        )
        sc_dihedrals_b_factor_mask = self._chi_mask_from_b_factors(
            protein["aatype"], protein["b_factors"]
        )
        sc_dihedrals_sincos = self._sincos_encoding(sc_dihedrals)
        sc_dihedrals_bin, sc_dihedrals_bin_offset = self._dihedral_bin_encoding(
            sc_dihedrals, n_bins=36, return_offset=True
        )
    ...
    def _adjust_features_for_residue_index(self, protein: FeatureDict) -> FeatureDict:
        ...
        atom_positions = np.full(
            (adj_len, *protein["atom_positions"].shape[1:]),
            fill_value=np.nan,
            dtype=np.float32,
        )
        ...
        atom_mask = np.zeros(
            (adj_len, *protein["atom_mask"].shape[1:]), dtype=np.float32
        )
    ...
    def _calc_dihedrals(atom_positions: Array, eps: float = 1e-8) -> Array:
        ...
        if isinstance(atom_positions, np.ndarray):
            ...
            dihedral = np.sign(np.sum(uvec_2 * nvec_1, axis=-1)) * np.arccos(
                cos_dihedral
            )
        else:
        ...
        if isinstance(atom_positions, np.ndarray):
            ...
            if residue_index is not None:
                ...
                pre_mask = np.concatenate(
                    (
                        [0.0],
                        (residue_index[1:] - 1 == residue_index[:-1]).astype(
                            np.float32
                        ),
                    ),
                    axis=-1,
            ...
            if residue_index is not None:
                ...
                bb_dihedrals_mask = torch.stack(
                    (pre_mask, post_mask, post_mask), axis=-1
                )
    ...
    def calc_sc_dihedrals(
        ...
        if isinstance(atom_positions, np.ndarray):
            ...
            chi_atom_positions = np.take_along_axis(
                atom_positions, chi_atom_indices[..., None].repeat(3, axis=-1), axis=-2
            )
        else:
    ...
    @staticmethod
    def chi_mask_from_b_factors(
        aatype: np.ndarray, b_factors: np.ndarray
    ) -> np.ndarray:
        ...
        chi_atom_indices = np.stack(
            [
                chi_atom_indices[:, :4],
                chi_atom_indices[:, 1:5],
                chi_atom_indices[:, 2:6],
                chi_atom_indices[:, 3:],
            ],
            axis=-1,
        )
        ...
        chi_atom_b_factors = np.take_along_axis(
            b_factors[..., None].repeat(4, axis=-1), chi_atom_indices, axis=-2
        )
...
def chi_mask_from_b_factors(
    ...
    # Get b_factors at all chi atoms
    chi_atom_b_factors = torch.gather(
        b_factors[..., None].expand(*aatype.shape, -1, 4), -2, chi_atom_indices.long()
    )
