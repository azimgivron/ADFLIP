        ...
        for res in exclude_res:
            if "-" in res:
                ...
                ppdb.df["ATOM"] = ppdb.df["ATOM"][
                    (ppdb.df["ATOM"]["residue_number"] < int(start))
                    | (ppdb.df["ATOM"]["residue_number"] > int(end))
                ]
            else:
                ppdb.df["ATOM"] = ppdb.df["ATOM"][
                    ppdb.df["ATOM"]["residue_number"] != int(res)
                ]
...
def main(full_target_dir, output_dir):
    ...
    full_target_files = [
        f for f in full_target_files if f.split(".")[0] in full_target_to_target
    ]
    ...
    for full_target_file in full_target_files:
        ...
        for target, chain, exclude_res in zip(
            target_info["target"], target_info["chain"], target_info["exclude_res"]
        ):
            ...
            extract_target(
                os.path.join(full_target_dir, full_target_file),
                output_file,
                chain,
                exclude_res,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "full_target_dir", help="Directory containing full target PDB files"
    )
    parser.add_argument(
        "output_dir", help="Directory to output extracted target PDB files"
    )
