...
class Top2018Dataset(Dataset):
    ...
    def download(self) -> None:
        ...
        pdb_tarball = download_file(
            version_info["pdb_tarball_url"],
            self.path,
            md5=version_info["pdb_tarball_md5"],
        )
        pdb_dir = os.path.join(self.path, os.path.basename(pdb_tarball)[:-7])
        if (
            not os.path.isdir(pdb_dir)
            or dir_size(pdb_dir) != version_info["pdb_dir_size"]
        ):
    ...
    def __getitem__(
        ...
        protein.update(
            vars(from_pdb_file(pdb_file, chain_id=protein["chain_id"], mse_to_met=True))
        )
    ...
    def _prune_from_cluster_split(
        ...
        subset_pdb_files = [
            self.pdb_files[self.protein_id_to_index[protein_id]]
            for protein_id in subset_ids
        ...
        # Update clusters
        self.clusters_to_protein_id[seq_id] = {
            cluster: self.clusters_to_protein_id[seq_id][cluster]
            for cluster in cluster_subset
            if cluster in self.clusters_to_protein_id[seq_id]
        }
    ...
    def _prune_from_removal_list(self, removal_list: Sequence[str]) -> None:
        ...
        subset_protein_ids = [
            ...
            if protein_id not in removal_list
        ]

        # Extract subset of pdb files
        subset_pdb_files = [
            self.pdb_files[self.protein_id_to_index[protein_id]]
            for protein_id in subset_protein_ids
            if protein_id in self.protein_id_to_index
        ]
        ...
        for seq_id in self.clusters_to_protein_id:
            for cluster_id in self.clusters_to_protein_id[seq_id]:
                ...
                new_cluster = [
                    protein for protein in old_cluster if protein not in removal_list
                ]
...
def transform_structure(
        ...
        if protein["aatype"].shape[0] > crop_size:
            ...
            if random_truncate:
                start_idx = random.choice(
                    list(range(protein["aatype"].shape[0] - crop_size + 1))
                )
    ...
    if len(np.unique(protein["chain_index"])) > 1:
        ...
        for chain_idx in np.unique(protein["chain_index"])[:-1]:
            index_offset += max(
                protein["residue_index"][protein["chain_index"] == chain_idx]
            )
            index_offset += 100
            protein["residue_index"][
                protein["chain_index"] == chain_idx + 1
            ] += index_offset
    ...
    BB_D, BB_D_mask = Featurizer.calc_bb_dihedrals(
        protein["atom_positions"], protein["residue_index"]
    )
    SC_D, SC_D_mask = Featurizer.calc_sc_dihedrals(
    ...
    SC_D_bin = np.nan_to_num(
        SC_D_bin
    )  # Convert nans to 0 before masking and converting to int64
    SC_D_bin_offset = (SC_D + np.pi) % (2 * np.pi / n_chi_bin)
    SC_D_bin = SC_D_bin * SC_D_mask + n_chi_bin * (1.0 - SC_D_mask)
...
def collate_fn(protein_batch):
    ...
    def _maybe_pad(protein, attr):
        ...
        return F.pad(
            attr_tensor,
            [0, 0] * (len(attr_tensor.shape) - 1) + [0, max_size - protein.num_nodes],
        )
    ...
    batch = Data(
        ...
        S=torch.stack(
            [_maybe_pad(protein, "S") for protein in protein_batch]
        ),  # [B, L]
        X=torch.stack(
            [_maybe_pad(protein, "X") for protein in protein_batch]
        ),  # [B, L, 14, 3]
        X_mask=torch.stack(
            [_maybe_pad(protein, "X_mask") for protein in protein_batch]
        ),  # [B, L, 14]
        residue_index=torch.stack(
            [_maybe_pad(protein, "residue_index") for protein in protein_batch]
        ),  # [B, L]
        og_residue_index=torch.stack(
            [_maybe_pad(protein, "og_residue_index") for protein in protein_batch]
        ),  # [B, L]
        chain_index=torch.stack(
            [_maybe_pad(protein, "chain_index") for protein in protein_batch]
        ),  # [B, L]
        residue_mask=torch.stack(
            [_maybe_pad(protein, "residue_mask") for protein in protein_batch]
        ),  # [B, L]
        BB_D=torch.stack(
            [_maybe_pad(protein, "BB_D") for protein in protein_batch]
        ),  # [B, L, 3]
        BB_D_sincos=torch.stack(
            [_maybe_pad(protein, "BB_D_sincos") for protein in protein_batch]
        ),  # [B, L, 3, 2]
        BB_D_mask=torch.stack(
            [_maybe_pad(protein, "BB_D_mask") for protein in protein_batch]
        ),  # [B, L, 3]
        SC_D=torch.stack(
            [_maybe_pad(protein, "SC_D") for protein in protein_batch]
        ),  # [B, L, 4]
        SC_D_sincos=torch.stack(
            [_maybe_pad(protein, "SC_D_sincos") for protein in protein_batch]
        ),  # [B, L, 4, 2]
        SC_D_bin=torch.stack(
            [_maybe_pad(protein, "SC_D_bin") for protein in protein_batch]
        ),  # [B, L, 4]
        SC_D_bin_offset=torch.stack(
            [_maybe_pad(protein, "SC_D_bin_offset") for protein in protein_batch]
        ),  # [B, L, 4]
        SC_D_mask=torch.stack(
            [_maybe_pad(protein, "SC_D_mask") for protein in protein_batch]
        ),  # [B, L, 4]
        SC_D_BF_mask=torch.stack(
            [_maybe_pad(protein, "SC_D_BF_mask") for protein in protein_batch]
        ),  # [B, L, 4]
    )
...
class Top2018DataModule(LightningDataModule):
    def __init__(
        ...
        if self.hparams.transform is None:
            self.transform = partial(
                transform_structure,
                n_chi_bin=self.hparams.n_chi_bins,
                crop_size=self.hparams.crop_size,
                random_truncate=self.hparams.random_truncate,
            )
    ...
    def setup(self, stage: str) -> None:
        ...
        if stage in ["fit", "all"]:
            ...
            self.data_train._prune_from_cluster_split(
                self.hparams.seq_id, data_split["train"]
            )
            if self.hparams.num_chains > 0 and self.hparams.num_chains < len(
                self.data_train.pdb_files
            ):
                self.data_train.pdb_files = self.data_train.pdb_files[
                    : self.hparams.num_chains
                ]
                self.data_train.protein_id_to_index = (
                    self.data_train.get_id_to_index_mapping()
                )
                self.data_train.clusters_to_protein_id = (
                    self.data_train.get_cluster_to_id_mapping()
                )
        if stage in ["fit", "validate", "all"]:
            # Validation data
            self.data_val = copy.deepcopy(top2018)
            self.data_val._prune_from_cluster_split(
                self.hparams.seq_id, data_split["valid"]
            )
            if self.hparams.num_chains > 0 and self.hparams.num_chains < len(
                self.data_val.pdb_files
            ):
                self.data_val.pdb_files = self.data_val.pdb_files[
                    : self.hparams.num_chains
                ]
                self.data_val.protein_id_to_index = (
                    self.data_val.get_id_to_index_mapping()
                )
                self.data_val.clusters_to_protein_id = (
                    self.data_val.get_cluster_to_id_mapping()
                )
        if stage in ["test", "all"]:
            # Testing data
            self.data_test = copy.deepcopy(top2018)
            self.data_test._prune_from_cluster_split(
                self.hparams.seq_id, data_split["test"]
            )
            if self.hparams.num_chains > 0 and self.hparams.num_chains < len(
                self.data_test.pdb_files
            ):
                self.data_test.pdb_files = self.data_test.pdb_files[
                    : self.hparams.num_chains
                ]
                self.data_test.protein_id_to_index = (
                    self.data_test.get_id_to_index_mapping()
                )
                self.data_test.clusters_to_protein_id = (
                    self.data_test.get_cluster_to_id_mapping()
                )
