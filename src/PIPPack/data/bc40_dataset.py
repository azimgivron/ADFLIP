...
class BC40Dataset(Dataset):
    ...
    def download(self) -> None:
        ...
        ss3_file = download_file(
            self.ss3_file_info["url"], self.path, md5=self.ss3_file_info["md5"]
        )
        ss3_dir = os.path.join(self.path, os.path.basename(ss3_file)[:-7])
        if (
            not os.path.isdir(ss3_dir)
            or dir_size(ss3_dir) != self.ss3_file_info["dir_size"]
        ):
            _ = extract(ss3_file)
        ...
        self.protein_ids = [
            protein[:4].upper() + "_" + protein[4] for protein in protein_ids
        ]
        self.protein_ids = [
            protein_id
            for protein_id in self.protein_ids
            if protein_id[:4] not in self.pdb_dir_info["bad_pdbs"]
        ]
        self.protein_ids = [
            protein_id
            for protein_id in self.protein_ids
            if protein_id not in self.pdb_dir_info["bad_chains"]
        ]
        ...
        if (
            ...
            or dir_size(pdb_dir) != self.pdb_dir_info["dir_size"]
        ):
            os.makedirs(pdb_dir, exist_ok=True)
            for protein_id in self.protein_ids:
                pdb_id = protein_id[:4]
                _ = download_file(
                    f"https://files.rcsb.org/download/{pdb_id}.pdb.gz", pdb_dir
                )
                self.protein_id_to_pdb_file[protein_id] = os.path.join(
                    pdb_dir, f"{pdb_id}.pdb.gz"
                )
        else:
            for protein_id in self.protein_ids:
                pdb_id = protein_id[:4]
    ...
    def __getitem__(
        ...
        protein.update(
            vars(from_pdb_file(pdb_file, chain_id=protein_id[-1], mse_to_met=True))
        )
    ...
    def _prune_from_id_split(self, id_subset: Sequence[str]) -> None:
        ...
        self.protein_ids = [
            protein_id for protein_id in id_subset if protein_id in self.protein_ids
        ]
        self.protein_id_to_pdb_file = {
            protein_id: self.protein_id_to_pdb_file[protein_id]
...
class BC40DataModule(LightningDataModule):
    ...
    def data_dir(self) -> str:
        return os.path.expanduser(os.path.join(self.hparams.data_dir, "bc40"))
