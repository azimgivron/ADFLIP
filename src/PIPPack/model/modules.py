...
try:
    ...
    from PIPPack.data.features import (
        frames_and_literature_positions_to_atom14_pos,
        get_bb_frames,
        torsion_angles_to_frames,
    )
    from PIPPack.model.loss import (
        BlackHole,
        local_interresidue_sc_clash_loss,
        nll_chi_loss,
        offset_mse,
        rotamer_recovery_from_coords,
        sc_rmsd,
        supervised_chi_loss,
        unclosed_proline_loss,
    )
except:
    import PIPPack.data.residue_constants as rc
    from PIPPack.data.features import (
                ...
                loss_fns.update(
                    {
                        "offset_mse_loss": lambda: offset_mse(
                            output["chi_bin_offset"],
                            batch.SC_D_bin_offset,
                            SC_D_mask,
                            self.n_chi_bins,
                            False,
                            _metric=_logger.get_metric(
                                _log_prefix + " offset mse loss"
                            ),
                        )
                    }
                )
    ...
    def compute_loss(
        ...
        loss_fns = {
            ...
            "clash_loss": lambda: local_interresidue_sc_clash_loss(
                ...
                output["gumbel_X"],
                0.6,
                _metric=_logger.get_metric(_log_prefix + " clash loss"),
            )["mean_loss"],
