...
def determine_best_epoch_from_log(
    ...
    header_cols = [
        header for header in log_lines[0].lower().strip().split(delimiter) if header
    ]
    ...
    for metric in metrics:
        if metric.lower() in header_cols:
            ...
            raise ValueError(
                f"Desired metric {metric} not found in the log file. Please choose one of {header_cols}."
            )

    # Provide equal weighting if no weights provided.
    if metric_weights == None:
        metric_weights = [1.0] * len(metrics)
    else:
        if len(metrics) != len(metric_weights):
            raise ValueError(
                f"The number of metrics must equal the number of weights. {len(metrics)} != {len(metric_weights)}."
            )
    ...
    for epoch_line in log_lines[1:]:
        if epoch_line:
            ...
            epoch_score = sum(
                [weight * val for val, weight in zip(metric_vals, metric_weights)]
            )
