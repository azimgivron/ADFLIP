...
def replace_protein_sequence(protein, protein_name, new_seqs):
    ...
    for i, seqs in enumerate(new_seqs):
        ...
        if len(seqs[0]) != len(protein['aatype']):
            raise ValueError(f"New sequence length ({len(seqs[0])}) does not match original sequence length ({len(protein['aatype'])}).")
        
        # Replace protein sequence info
        new_protein = copy.deepcopy(protein)

        aatype = np.array([rc.restype_order[res] for res in new_seqs[0][0] if res ]).astype(np.int64)
