...
try:
    ...
    from PIPPack.data.features import atom14_to_atom37, make_atom14_masks
    from PIPPack.data.protein import Protein, from_pdb_file, to_pdb
...
def replace_protein_sequence(protein, protein_name, new_seqs):
    ...
    for i, seqs in enumerate(new_seqs):
        ...
        if len(seqs[0]) != len(protein["aatype"]):
            raise ValueError(
                f"New sequence length ({len(seqs[0])}) does not match original sequence length ({len(protein['aatype'])})."
            )
    ...
    if resample:
        for i in range(batch.S.shape[0]):
            ...
            protein = {
                ...
                "chi_bin_offset": (
                    results["chi_bin_offset"][i]
                    if "chi_bin_offset" in results
                    else None
                ),
            }
...
def main(cfg: DictConfig) -> None:
    ...
    checkpoint = os.path.join(
        cfg.inference.weights_path, f"{cfg.inference.model_name}_ckpt.pt"
    )
    ...
    if cfg.inference.get("replace_seqs", False):
        ...
        with open(fasta_files[0], "r") as f:
            lines = f.readlines()
        new_seqs = [
            line.strip().split("/") for line in lines if line[0] != ">" and line
        ]
        print(new_seqs)
        proteins = replace_protein_sequence(
            vars(from_pdb_file(pdb_files[0], mse_to_met=True)),
            os.path.basename(pdb_files[0])[:-4],
            new_seqs,
        )
    else:
        proteins = [
    ...
    proteins = [
        (
            protein[0],
            transform_structure(
                protein[1], exp_cfg.model.n_chi_bins, sc_d_mask_from_seq=True
            ),
        )
    ...
    for batch in batches:
        ...
        sample_results = sample_epoch(
            ...
            device,
            n_recycle=cfg.inference.n_recycle,
            resample=cfg.inference.use_resample,
            resample_args=cfg.inference.resample_args,
        )
